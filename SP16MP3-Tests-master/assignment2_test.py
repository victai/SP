...
if not os.path.exists('./assignment2_test_result'):
	os.mkdir('./assignment2_test_result')
...
if pid_server == 0:
	#child process
	os.chdir('./bin')
	f = open(cwd+'/assignment2_test_result/csiebox_server_stdout','w+')
	os.dup2(f.fileno(),1)
	f = open(cwd+'/assignment2_test_result/csiebox_server_stderr','w+')
	os.dup2(f.fileno(),2)
...
if pid_child == 0:
	#child process
	os.chdir('./bin')
	f = open(cwd+'/assignment2_test_result/csiebox_client_stdout','w+')
	os.dup2(f.fileno(),1)
	f = open(cwd+'/assignment2_test_result/csiebox_client_stderr','w+')
	os.dup2(f.fileno(),2)
...
print '================================================================='
[type_wrong, exist_wrong, content_wrong, mode_wrong, time_wrong, right]= diff_dir('./cdir','./sdir/user')
print 'result after restart synchronization:'
print 'files that only exist in one side:'
if len(exist_wrong) == 0:
	print 'None'
else:
