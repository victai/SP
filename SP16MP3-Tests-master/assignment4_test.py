...
if not os.path.exists('./assignment4_test_result'):
	os.mkdir('./assignment4_test_result')
...
if pid_server == 0:
	#child process
	os.chdir('./bin')
	f = open(cwd+'/assignment4_test_result/csiebox_server_stdout','w+')
	os.dup2(f.fileno(),1)
	f = open(cwd+'/assignment4_test_result/csiebox_server_stderr','w+')
	os.dup2(f.fileno(),2)
...
for i in range(1,6,1):
	...
	if pid == 0:
		#child process
		os.chdir('./bin')
		f = open(cwd+'/assignment4_test_result/csiebox_client'+str(i) + '_stdout','w+')
		os.dup2(f.fileno(),1)
		f = open(cwd+'/assignment4_test_result/csiebox_client'+str(i) + '_stderr','w+')
		os.dup2(f.fileno(),2)
...
for i in range(0,5,2):
	...
	shutil.rmtree('./cdir_' + str(i+1))
	os.mkdir('./cdir_' + str(i+1))
	pid = os.fork()
	if pid == 0:
		#child process
		os.chdir('./bin')
		f = open(cwd+'/assignment4_test_result/csiebox_client'+str(i+1)+'_stdout','w+')
		os.dup2(f.fileno(),1)
		f = open(cwd+'/assignment4_test_result/csiebox_client'+str(i+1)+'_stderr','w+')
		os.dup2(f.fileno(),2)
		os.execl('./csiebox_client', 'client', '../config/client_' + str(i+1)+'.cfg')
	else:
		print 'restart client ' + str(i+1)
		pid_child[i] = pid
...
for i in range(0,5,1):
	[type_wrong, exist_wrong, content_wrong, mode_wrong, time_wrong, right]= diff_dir('./cdir_' + str(i+1),'./sdir/' + client_user_name[i])
	print '----------------------------------------------------------------'
        print 'result of client' + str(i+1) + ' after restart synchronization:'
	print 'files that only exist in one side:'
	if len(exist_wrong) == 0:
		print 'None'
	else:
