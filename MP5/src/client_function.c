...
int check_busy(int conn_fd){
	int status;
	recv_message(conn_fd, &status, sizeof(int));
	return status;
}
...
int client_send_meta( const char *path, const csiebox_client *client )
{
	...
	while(1){
		...
		int status = check_busy(client->conn_fd);
		if(status == 1){
			printf("Server busy\n");
			fflush(stdout);
			sleep(3);
...
int client_send_file( const char *path, const csiebox_client *client)
{
	...
	while(1){
		int ret = flock(file_fd, LOCK_SH | LOCK_NB);
		if(ret == 0)	break;
	}
	...
	do
	{
		nByte_read = read(file_fd,buf,65536);
		// fill file req
		...
		fprintf(stderr, "send file recv header\n");
		memset(&header, 0, sizeof(header));
		if (recv_message(client->conn_fd, &header, sizeof(header))) {
			if (header.res.magic == CSIEBOX_PROTOCOL_MAGIC_RES &&
